...
| Realizar check-in | `POST /estadia/checkin` | Elegir habitación física y confirmar check-in de una habitación completa (todos sus huéspedes ya asignados, titular incluido) | `app/db.py:call_procedures_en_transaccion` → `CALL sp_realizar_checkin(...)` + un `CALL sp_agregar_huesped_alojamiento(...)` por huésped, todo en una transacción | Trigger `trg_alojamiento_checkin` → habitación pasa a `OCUPADA`; rechaza si el huésped ya está activo en otro alojamiento |
| Alojamiento (hub) | `GET /estadia/<id_alojamiento>` | Ver huéspedes, consumos, daños | `SELECT` sobre `alojamiento`, `huesped_alojamiento`, `vw_consumos_alojamiento`, `servicio`, `danio`; `huespedes_disponibles` excluye a quien ya esté activo en otro alojamiento | — |
| Agregar huésped existente | `POST /estadia/<id>/huespedes` | Sumar huésped ya registrado al alojamiento | `CALL sp_agregar_huesped_alojamiento(...)` | Trigger `trg_huesped_alojamiento_capacidad` valida cupo; rechaza si el huésped ya está activo en otro alojamiento |
| Huésped nuevo (desde estadía) | `POST /estadia/huesped/nuevo` | Alta de huésped (siempre identificado) | `INSERT INTO persona` → `persona_natural` → `huesped` vía `app/huespedes.py:crear_huesped_desde_formulario` (o solo `huesped` si ya existe la persona con ese documento) | — |
