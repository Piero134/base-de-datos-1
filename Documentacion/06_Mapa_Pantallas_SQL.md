...
| Realizar check-in | `POST /estadia/checkin` | Check-in individual de UN huésped ya asignado (igual de individual que la salida). Si es el primero de la habitación, además elige la habitación física | Primer huésped de la habitación: `app/db.py:call_procedures_en_transaccion` → `CALL sp_realizar_checkin(...)` + `CALL sp_agregar_huesped_alojamiento(...)`, atómico. Siguientes huéspedes de la misma habitación: solo `CALL sp_agregar_huesped_alojamiento(...)` | Trigger `trg_alojamiento_checkin` → habitación pasa a `OCUPADA` en el primer check-in; rechaza si el huésped ya está activo en otro alojamiento |
