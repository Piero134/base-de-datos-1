...
| Detalle de reserva | `GET/POST /reservas/<id>/detalle` | Agregar línea (tipo de habitación) vía ventana flotante "+ Agregar línea" (`<dialog>`, mismo patrón que cliente/huésped nuevo; se reabre solo con lo tecleado si el SP rechaza el intento); si el estado es `CANCELADA`/`NO_SHOW`/`FINALIZADA`, la pantalla ya no ofrece ningún formulario/CTA, solo un aviso | `CALL sp_agregar_detalle_reserva(...)` (usa `fn_plan_vigente`, `fn_disponibilidad_tipo_habitacion` internamente; rechaza si `reserva.pagado = 1` o si el estado es final) | Recalcula `monto_total` |
| Editar línea | `POST /reservas/<id>/detalle/<id_detalle_reserva>/editar` | Botón "Editar" por línea (ventana flotante, no visible si la línea ya tiene check-in) | `CALL sp_editar_detalle_reserva(...)` (misma lógica de disponibilidad/plan/precio que agregar; además rechaza si ya tiene check-in o si la nueva cantidad/tipo ya no alcanza para los huéspedes pre-asignados) | Recalcula `monto_total` (trigger `trg_reserva_detalle_monto_au`) |
| Quitar línea | `POST /reservas/<id>/detalle/<id_detalle_reserva>/eliminar` | Botón "Quitar" por línea, con confirmación que avisa si arrastrará huéspedes pre-asignados | `CALL sp_eliminar_detalle_reserva(...)` (rechaza si ya tiene check-in) | Recalcula `monto_total` (trigger `trg_reserva_detalle_monto_ad`); cascada sobre `detalle_huesped_reserva` (`fk_dhr_detalle_reserva`) |
| Cancelar reserva | `POST /reservas/<id>/cancelar` (Recepción/Administrador) | Botón "Cancelar reserva" en el detalle, con confirmación (`data-confirm`); solo visible si el estado no es ya final | `CALL sp_cancelar_reserva(id_reserva)` | Rechaza si ya está en un estado final o si tiene huéspedes con `alojamiento.estado = 'ACTIVO'`; si no, `estado = 'CANCELADA'` |
| Marcar no-show | `POST /reservas/<id>/no-show` (Recepción/Administrador) | Botón "Marcar como no-show" en el detalle, con confirmación | `CALL sp_marcar_no_show(id_reserva)` | Rechaza si ya está en un estado final, si aún no pasó `fecha_checkin`, o si ya tuvo algún check-in real; si no, `estado = 'NO_SHOW'` |
| Confirmación de pago | `GET /reservas/<id>/pago` (solo Caja/Administrador) / `POST` (solo Caja/Administrador) | Pantalla exclusiva de Caja: Recepción no tiene ni el link (detalle) ni acceso a la ruta | `CALL sp_confirmar_pago(id_reserva)` (rechaza si el estado ya es final) | El propio SP hace `estado = 'CONFIRMADA'` (sin trigger de por medio) |
| Asignación de huéspedes | `GET /reservas/<id>/preasignar` | Ver la tabla de asignación (toda reserva, no solo corporativa): por línea, subdividida en habitaciones de `capacidad_base` cupos (calculado en Python, ver `app/asignacion_huespedes.py:construir_grid_reserva`); si el estado es final, la tabla se muestra de solo lectura (sin selects/botón de guardar) | `SELECT` sobre `reserva_detalle`+`tipo_habitacion`+`detalle_huesped_reserva` | — |
| Guardar asignación de una línea | `POST /reservas/<id>/asignacion/<id_detalle_reserva>` | Editar en la misma tabla todos los cupos de una línea a la vez (huésped + titular por habitación) | `INSERT`/`UPDATE` sobre `detalle_huesped_reserva` en una sola transacción; valida en Python que cada habitación con huéspedes tenga exactamente un titular, y que el estado de la reserva no sea final, antes de guardar | Trigger `trg_valida_cupos_reserva` limita el total de cupos por línea; las habitaciones ya con check-in se ignoran (solo lectura) |
| Huésped nuevo (desde reservas) | `POST /reservas/huesped/nuevo` | Alta de la persona natural que va a ocupar un cupo (siempre identificada) | `INSERT INTO persona` → `persona_natural` vía `app/huespedes.py:crear_huesped_desde_formulario` (reutiliza la persona si ya existe con ese documento; no hay tabla `huesped` — "huésped" es un rol, cualquier `persona_natural` activa es seleccionable) | — |

## Estadía (`app/estadia/routes.py`)

| Pantalla | Ruta Flask | Acción del usuario | SQL invocado | Efecto / trigger disparado |
|---|---|---|---|---|
| Alojamientos activos | `GET /estadia/activos` | Ver habitaciones ocupadas: una fila por habitación (no por huésped), con sus huéspedes agregados en una columna (titular primero) y botón directo al alojamiento | `SELECT * FROM vw_alojamientos_activos` (la vista agrupa por `id_alojamiento` con `GROUP_CONCAT`) | — |
...
| Check-in de una reserva | `GET /estadia/checkin/<id_reserva>` | Ver, por habitación (mismo grid que la asignación de reservas, `app/asignacion_huespedes.py:construir_grid_reserva` con `con_estado_estadia=True`), quién ya tiene check-in y quién tiene el botón de check-in habilitado; si el estado de la reserva ya es final, ningún cupo ofrece botón de check-in | `SELECT` sobre `reserva_detalle`, `tipo_habitacion`, `detalle_huesped_reserva`, `huesped_alojamiento`+`alojamiento`, `habitacion` | — |
| Realizar check-in | `POST /estadia/checkin` | Check-in individual de UN huésped ya asignado (igual de individual que la salida); el titular de la habitación debe ser el primero (rechazado si no lo es). Si es el primero de la habitación, además elige la habitación física | Primer huésped de la habitación (siempre el titular): `app/db.py:call_procedures_en_transaccion` → `CALL sp_realizar_checkin(...)` (rechaza si el estado de la reserva ya es final) + `CALL sp_agregar_huesped_alojamiento(...)`, atómico. Siguientes huéspedes de la misma habitación: solo `CALL sp_agregar_huesped_alojamiento(...)` | Trigger `trg_alojamiento_checkin` → habitación pasa a `OCUPADA` en el primer check-in; rechaza si el huésped ya está activo en otro alojamiento; trigger `trg_alojamiento_habitacion_unica` rechaza si la habitación ya tiene otro alojamiento `ACTIVO` (red de seguridad para inserts directos, `sp_realizar_checkin` ya lo valida antes); trigger `trg_reserva_finalizar` pasa la reserva a `FINALIZADA` cuando el checkout deja sin ningún alojamiento `ACTIVO` para esa reserva |
...
| Registrar consumo | `POST /estadia/<id>/consumo` | Cargar un servicio consumido, vía ventana flotante "+ Registrar consumo" (`<dialog>`, mismo patrón que cliente/huésped nuevo) | `CALL sp_registrar_consumo(...)` | Calcula subtotal con precio vigente |
| Registrar daño | `POST /estadia/<id>/danio` | Reportar daño en la habitación, vía ventana flotante "+ Registrar daño" (`<dialog>`) | `CALL sp_registrar_danio(...)` | — |
